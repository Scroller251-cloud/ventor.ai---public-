...
class ProviderRouter:
    """Reusable async transport, bounded timeouts, circuit breakers and fallback."""
    def __init__(self):
        ...
        if self._client is None or self._client.is_closed:
            async with self._client_lock:
                if self._client is None or self._client.is_closed:
                    ...
                    self._client = httpx.AsyncClient(timeout=timeout, limits=httpx.Limits(max_connections=64, max_keepalive_connections=16), follow_redirects=False)
        return self._client
    ...
    def status(self):
        ...
        for provider in ("ollama", "openai", "anthropic", "gemini"):
            ...
            result[provider] = {"configured": self._available(provider), "healthy": state.opened_until <= now, "failures": state.failures, "successes": state.successes, "last_latency_ms": state.last_latency_ms}
        return result
    ...
    async def _request(self, provider, method, url, **kwargs):
        ...
        await self._mark_success(provider, round((time.perf_counter() - started) * 1000))
        return response
    ...
    async def _call(self, provider, model, request_fn):
        ...
        try:
            ...
            return ProviderReply(provider, model, "", False, f"{type(exc).__name__}: {exc}", round((time.perf_counter() - started) * 1000))

    async def _ollama(self, prompt, system):
        model = os.getenv("VENTOR_CHAT_MODEL", "qwen3:4b")
        async def request():
            r = await self._request("ollama", "POST", self.ollama + "/api/generate", json={"model": model, "prompt": prompt, "system": system, "stream": False})
    ...
    async def _openai(self, prompt, system):
        ...
        async def request():
            r = await self._request("openai", "POST", "https://api.openai.com/v1/responses", headers={"Authorization": "Bearer " + self.openai_key, "Content-Type": "application/json"}, json={"model": model, "input": [{"role": "system", "content": system}, {"role": "user", "content": prompt}]})
    ...
    async def _anthropic(self, prompt, system):
        ...
        async def request():
            r = await self._request("anthropic", "POST", "https://api.anthropic.com/v1/messages", headers={"x-api-key": self.anthropic_key, "anthropic-version": "2023-06-01", "content-type": "application/json"}, json={"model": model, "max_tokens": 2048, "system": system, "messages": [{"role": "user", "content": prompt}]})
    ...
    async def _gemini(self, prompt, system):
        ...
        async def request():
            ...
            r = await self._request("gemini", "POST", url, json={"systemInstruction": {"parts": [{"text": system}]}, "contents": [{"role": "user", "parts": [{"text": prompt}]}]})
    ...
    async def generate(self, prompt, system, preferred=None):
        ...
        for provider in order:
            ...
            if self._state(provider).opened_until > time.monotonic():
                ...
                continue
            reply = {"ollama": self._ollama, "openai": self._openai, "anthropic": self._anthropic, "gemini": self._gemini}[provider]
            result = await reply(prompt, system)
            if result.ok:
                return result
