...
async def open_page(url: str, *, timeout_ms: int = 15000) -> dict:
    ...
    try:
        ...
        raise RuntimeError("Playwright is not installed; install playwright and Chromium") from exc
    async with async_playwright() as playwright:
        ...
        try:
            ...
            links = await page.locator("a[href]").evaluate_all("els => els.slice(0, 30).map(a => ({text:(a.innerText||'').trim(), href:a.href}))")
            return {"url": page.url, "status": response.status if response else None, "title": await page.title(),
                    "text": _clean_text(await page.locator("body").inner_text()), "links": links}
        finally:
            await browser.close()
