...
@app.get("/health")
def health():
    return {"status": "ok", "version": APP_VERSION, "service": "ventor", "auto_upgrade": False}


@app.get("/ready")
def readiness():
    checks = {"runtime": True, "auth_roster": bool(os.getenv("VENTOR_AUTH_ROSTER"))}
    ready = all(checks.values()) if os.getenv("VENTOR_PRODUCTION", "0") == "1" else True
    from fastapi.responses import JSONResponse
    return JSONResponse({"status": "ready" if ready else "not_ready", "version": APP_VERSION, "checks": checks},
                        status_code=200 if ready else 503)


@app.get("/metrics")
def metrics():
    return {"version": APP_VERSION, "runtime": runtime.snapshot(), "models": model_manager.snapshot()}
...
async def verify(t: Task):
    ...
    return {"version": APP_VERSION, "results": [r.__dict__ for r in results], "evidence_checks": evidence,
            "verdict": verdict.__dict__, "owner_approval_required": True, "upgrade_allowed": False}
...
async def learn(t: LearnTask):
    ...
    return {"version": APP_VERSION, **result, "owner_approval_required": True, "core_modified": False,
            "message": "Verified lessons may inform candidate improvements; core changes require owner approval."}


@app.get("/api/learning/status", dependencies=[Depends(require_team_or_owner)])
def learning_status():
...
@app.get("/api/models")
def models():
    return {"agents": public_registry(), "runtime": model_manager.snapshot()}
...
def code_security(t: CodeTest):
    return {"version": APP_VERSION, "security": security_check(t.code), "owner_approval_required": False,
            "note": "Only the restricted deterministic subset may execute; arbitrary generated code is rejected."}


@app.get("/api/selftest", dependencies=[Depends(require_team_or_owner)])
async def selftest():
    results = await run_stress_tests()
    return {"version": APP_VERSION, "passed": all(x["passed"] for x in results), "tests": results,
            "owner_gate": "external_immutable", "auto_upgrade": False}
