...
def _text(html: str, limit: int = 9000) -> str:
    ...
    return " ".join(unescape(re.sub(r"<[^>]+>", " ", html)).split())[:limit]


async def _fetch_public_html(client, url):
...
async def research(query: str, limit: int = 5) -> dict:
    ...
    sources = []
    async with httpx.AsyncClient(timeout=10, headers={"User-Agent": "Ventor/3.0"}, limits=httpx.Limits(max_connections=8, max_keepalive_connections=4)) as client:
        for item in results:
            url = item.get("url")
            ...
            try:
                ...
                continue
    return {"query": query, "sources": sources, "source_count": len(sources), "note": "Sources are evidence only; Ventor must distinguish evidence from inference."}
